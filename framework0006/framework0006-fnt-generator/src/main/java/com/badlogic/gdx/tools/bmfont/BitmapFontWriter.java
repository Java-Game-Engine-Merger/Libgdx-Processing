 ...
 * from TrueTypeFont, for faster load
 * times.
 * <p>
 ...
 * {@Link com.badlogic.gdx.tools.hiero.Hiero}.
 * BitmapFontWriter gives more flexibility, eg borders and shadows can be used. Hiero is able to
 * avoid outputting the same glyph
...
public class BitmapFontWriter{
  ...
  private static String quote(Object params,boolean spaceAfter) {
    if(BitmapFontWriter.getOutputFormat()==OutputFormat.XML) return "\""+params.toString().trim()+"\""+(spaceAfter?" ":"");
    else return params.toString();
  }

  /**
   * Writes the given BitmapFontData to a file, using the specified <tt>pageRefs</tt> strings as
   * the image paths for each
   * texture page. The glyphs in BitmapFontData have a "page" id, which references the index of
   * the pageRef you specify here.
   * 
   * The FontInfo parameter is useful for cleaner output; such as including a size and font face
   * name hint. However, it can be
   * null to use default values. libGDX ignores most of the "info" line when reading back fonts,
   * only padding is used. Padding
   ...
   * libGDX's reader. For maximum
   * compatibility with other BMFont tools, you should use the width and height of your texture
   * pages (each page should be the
   * same size).
   * 
   * @param fontData   the bitmap font
   * @param pageRefs   the references to each texture page image file, generally in the same
   *                   folder as outFntFile
