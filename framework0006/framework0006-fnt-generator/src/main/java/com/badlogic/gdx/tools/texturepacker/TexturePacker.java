...
public class TexturePacker{
  ...
  public TexturePacker(File rootDir,Settings settings) {
    ...
    if(settings.multipleOfFour) {
      if(settings.maxWidth%4!=0) throw new RuntimeException("If mod4 is true, maxWidth must be evenly divisible by 4: "+settings.maxWidth);
      if(settings.maxHeight%4!=0) throw new RuntimeException("If mod4 is true, maxHeight must be evenly divisible by 4: "+settings.maxHeight);
    }
  ...
  public void pack(File outputDir,String packFileName) {
    if(packFileName.endsWith(settings.atlasExtension)) packFileName=packFileName.substring(0,packFileName.length()-settings.atlasExtension.length());
    outputDir.mkdirs();
    ...
    for(int i=0;i<n;i++) {
      ...
      if(settings.scaleResampling!=null&&settings.scaleResampling.length>i&&settings.scaleResampling[i]!=null) imageProcessor.setResampling(settings.scaleResampling[i]);
      ...
      for(int ii=0,nn=inputImages.size;ii<nn;ii++,progress.count++) {
        ...
        if(inputImage.file!=null) imageProcessor.addImage(inputImage.file,inputImage.rootPath);
  ...
  private void writePackFile(File outputDir,String scaledPackFileName,Array<Page> pages) throws IOException {
    ...
    for(int i=0,n=pages.size;i<n;i++) {
      ...
      if(settings.legacyOutput) writePageLegacy(writer,page);
      else {
      ...
      for(Rect rect:page.outputRects) {
        if(settings.legacyOutput) writeRectLegacy(writer,page,rect,rect.name);
        else writeRect(writer,page,rect,rect.name);
        ...
        for(Alias alias:aliases) {
          ...
          if(settings.legacyOutput) writeRectLegacy(writer,page,aliasRect,alias.name);
  ...
  static public class Rect implements Comparable<Rect>{
    ...
    /**
     * Clears the image for this rect, which will be loaded from the specified file by
     * {@link #getImage(ImageProcessor)}.
     */
  ...
  /**
   * @return true if the output file does not yet exist or its last modification date is before
  ...
  static private boolean isModified(File file,long lastModified) {
    ...
    if(children!=null) {
      for(File child:children) if(isModified(child,lastModified)) return true;
    }
    return false;
  }
