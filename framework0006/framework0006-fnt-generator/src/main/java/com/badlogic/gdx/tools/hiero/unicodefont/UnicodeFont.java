...
// BOZO - Look at actual pixels to determine glyph size, current size sometimes selects blank
// pixels (eg Calibri, 45, 'o').

/**
 * A bitmap font that can display unicode glyphs from a TrueTypeFont.
 * 
 * For efficiency, glyphs are packed on to textures. Glyphs can be loaded to the textures on the
 * fly, when they are first needed
...
public class UnicodeFont{
  ...
  /**
   * Queues the glyphs in the specified codepoint range (inclusive) to be loaded. Note that the
   * glyphs are not actually loaded
   * until {@link #loadGlyphs()} is called.
   * 
   * Some characters like combining marks and non-spacing marks can only be rendered with the
   * context of other glyphs. In this
  ...
  /**
   * Queues the glyphs in the specified text to be loaded. Note that the glyphs are not actually
   * loaded until
  ...
  /**
   * Queues the glyphs in the ASCII character set (codepoints 32 through 255) to be loaded. Note
   * that the glyphs are not
  ...
  /**
   * Queues the glyphs in the NEHE character set (codepoints 32 through 128) to be loaded. Note
   * that the glyphs are not actually
  ...
  /**
   * Loads all queued glyphs to the backing textures. Glyphs that are typically displayed together
   * should be added and loaded at
   * the same time so that they are stored on the same backing texture. This reduces the number of
   * backing texture binds required
   * to draw glyphs.
   */
  ...
  /**
   * Loads up to the specified number of queued glyphs to the backing textures. This is typically
   * called from the game loop to
  ...
  /**
   * Releases all resources used by this UnicodeFont. This method should be called when this
   * UnicodeFont instance is no longer
   * needed.
   */
  public void dispose() {
    for(Iterator iter=glyphPages.iterator();iter.hasNext();) {
  ...
  /**
   * Returns the glyph for the specified codePoint. If the glyph does not exist yet, it is created
   * and queued to be loaded.
   */
  ...
  private Rectangle getGlyphBounds(GlyphVector vector,int index,int codePoint) {
    ...
    if(renderType==RenderType.Native) {
      ...
      else bounds=metrics.getStringBounds(""+(char)codePoint,GlyphPage.scratchGraphics).getBounds();
    }
  ...
  /**
   * Returns the padding to the right of a glyph on the GlyphPage to allow for effects to be
   * drawn.
   */
  ...
  /**
   * Sets the additional amount to offset glyphs on the x axis. This is typically set to a
   * negative number when left or right
  ...
  /**
   * Sets the additional amount to offset a line of text on the y axis. This is typically set to a
   * negative number when top or
  ...
  /**
   * Returns the distance from one line of text to the next. This is the sum of the descent,
   * ascent, leading, padding top,
   * padding bottom, and padding advance y. To change the line height, use
  ...
  /**
   * Gets the distance from the baseline to the bottom of most alphanumeric characters with
   * descenders.
   */
  ...
  public void setRenderType(RenderType renderType) {
    ...
    if(renderType!=RenderType.FreeType) {
      ...
      if(fontFile!=null) {
        ...
        if(bitmapFont.getData().missingGlyph==null) bitmapFont.getData().missingGlyph=bitmapFont.getData().getGlyph('\ufffd');
  ...
  /**
   * Returns the path to the TTF file for this UnicodeFont, or null. If this UnicodeFont was
   * created without specifying the TTF
