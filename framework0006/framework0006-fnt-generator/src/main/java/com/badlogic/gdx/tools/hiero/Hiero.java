...
/**
 * A tool to visualize settings for {@link UnicodeFont} and to export BMFont files for use with
 * {@link BitmapFont}.
 * <p>
 * This tool is quite old. It was originally written for Slick, then ported to libgdx. It has
 * had a few different contributors over the years, the FreeType rendering was tacked on and in
 * general the code isn't great. It does its job though!
 * 
 * @author Nathan Sweet
 */
public class Hiero extends JFrame{
  ...
  void open(File file) {
    ...
    else if(settings.getRenderType()==RenderType.FreeType.ordinal()) freeTypeRadio.setSelected(true);
  ...
  private class EffectPanel extends JPanel{
    ...
    public void addValue(final Value value) {
      ...
      if(value.getObject() instanceof java.awt.Color) valueValueLabel.setIcon(getColorIcon((java.awt.Color)value.getObject()));
