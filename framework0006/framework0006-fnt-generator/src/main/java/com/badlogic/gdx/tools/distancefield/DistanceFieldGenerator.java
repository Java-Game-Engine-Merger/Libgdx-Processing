 ...
 * 
 * <p>
 * Signed distance fields are used in Team Fortress 2 by Valve to enable sharp rendering of
 * bitmap fonts even at high
 * magnifications, using nothing but alpha testing so at no extra runtime cost.
 * 
 * <p>
 * The technique is described in the SIGGRAPH 2007 paper "Improved Alpha-Tested Magnification
 * for Vector Textures and Special
 * Effects" by Chris Green: <a href=
...
public class DistanceFieldGenerator{
  ...
  /**
   * Sets the color to be used for the output image. Its alpha component is ignored. Defaults to
   * white, which is convenient for
  ...
  /**
   * Sets the factor by which to downscale the image during processing. The output image will be
   * smaller than the input image by
   * this factor, rounded downwards.
   * 
   * <p>
   * For greater accuracy, images to be used as input for a distance field are often generated at
   * higher resolution.
   * 
  ...
  /**
   * Sets the spread of the distance field. The spread is the maximum distance in pixels that
   * we'll scan while for a nearby
   ...
   * 
   * The returned image is a factor of {@code upscale} smaller than {@code inImage}. Opaque pixels
   * more than {@link #spread} away
   * in the output image from white remain opaque; transparent pixels more than {@link #spread}
   * away in the output image from
   * black remain transparent. In between, we get a smooth transition from opaque to transparent,
   * with an alpha value of 0.5 when
  ...
  /**
   * Returns {@code true} if the color is considered as the "inside" of the image, {@code false}
   * if considered "outside".
   * 
   * <p>
   * Any color with one of its color channels at least 128 <em>and</em> its alpha channel at least
   * 128 is considered "inside".
   */
  ...
  /**
   * For a distance as returned by {@link #findSignedDistance}, returns the corresponding "RGB"
   * (really ARGB) color value.
   * 
   ...
   * 
   * For points "inside", this is the distance to the closest "outside" pixel. For points
   * "outside", this is the
   * <em>negative</em> distance to the closest "inside" pixel. If no pixel of different color is
  ...
  /**
   * Main function to run the generator as a standalone program. Run without arguments for usage
   * instructions (or see
   * {@link #usage()}).
   * 
