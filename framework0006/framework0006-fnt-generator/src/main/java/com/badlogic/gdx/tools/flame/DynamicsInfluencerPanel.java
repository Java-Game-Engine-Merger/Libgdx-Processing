...
public class DynamicsInfluencerPanel extends InfluencerPanel<DynamicsInfluencer> {
	...
	private DynamicsModifier createVelocityValue (Object selectedItem) {
		...
		if (selectedItem == VEL_TYPE_ROTATIONAL_2D)
			...
			velocityValue = new Rotational3D();
		else if (selectedItem == VEL_TYPE_CENTRIPETAL)
			velocityValue = new CentripetalAcceleration();
		else if (selectedItem == VEL_TYPE_TANGENTIAL)
			velocityValue = new TangentialAcceleration();
		else if (selectedItem == VEL_TYPE_POLAR)
			velocityValue = new PolarAcceleration();
		else if (selectedItem == VEL_TYPE_BROWNIAN)
			velocityValue = new BrownianAcceleration();
