...
public class DynamicsInfluencerPanel extends InfluencerPanel<DynamicsInfluencer>{
  ...
  private EditorPanel getVelocityPanel(DynamicsModifier velocityValue) {
    ...
    if(velocityValue instanceof DynamicsModifier.Rotational2D) {
      ...
      strengthVelocityPanel.setValue((CentripetalAcceleration)velocityValue);
  ...
  private DynamicsModifier createVelocityValue(Object selectedItem) {
    ...
    else if(selectedItem==VEL_TYPE_ROTATIONAL_3D) velocityValue=new Rotational3D();
    else if(selectedItem==VEL_TYPE_CENTRIPETAL) velocityValue=new CentripetalAcceleration();
    else if(selectedItem==VEL_TYPE_TANGENTIAL) velocityValue=new TangentialAcceleration();
    else if(selectedItem==VEL_TYPE_POLAR) velocityValue=new PolarAcceleration();
    else if(selectedItem==VEL_TYPE_BROWNIAN) velocityValue=new BrownianAcceleration();
    else if(selectedItem==VEL_TYPE_FACE) velocityValue=new FaceDirection();
