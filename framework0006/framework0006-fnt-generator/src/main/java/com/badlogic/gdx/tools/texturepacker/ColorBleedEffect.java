...
public class ColorBleedEffect{
  ...
  public BufferedImage processImage(BufferedImage image,int maxIterations) {
    ...
    if(image.getType()==BufferedImage.TYPE_INT_ARGB) processedImage=image;
    else processedImage=new BufferedImage(width,height,BufferedImage.TYPE_INT_ARGB);
  ...
  static private int argb(int a,int r,int g,int b) {
    if(a<0||a>255||r<0||r>255||g<0||g>255||b<0||b>255) throw new IllegalArgumentException("Invalid RGBA: "+r+", "+g+","+b+","+a);
  ...
  static private class Mask{
    ...
    class MaskIterator{
      ...
      void reset() {
        index=0;
        for(int i=0,n=changingSize;i<n;i++) blank[changing[i]]=false;
        changingSize=0;
      }
    }
