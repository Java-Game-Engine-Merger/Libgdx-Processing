...
/**
 * Packs pages of images using the maximal rectangles bin packing algorithm by Jukka Jylänki. A
 * brute force binary search is used
...
public class MaxRectsPacker implements Packer{
  ...
  private Page packPage(Array<Rect> inputRects) {
    ...
    for(int i=0,nn=inputRects.size;i<nn;i++) {
      ...
      if(settings.rotation) {
        if((width>maxWidth||height>maxHeight)&&(width>maxHeight||height>maxWidth)) {
          ...
          throw new RuntimeException("Image does not fit within max page size "+settings.maxWidth+"x"+settings.maxHeight+paddingMessage+": "+rect.name+" "+width+"x"+height);
        }
      }else {
        if(width>maxWidth) {
          String paddingMessage=edgePadX?(" and X edge padding "+paddingX+"*2"):"";
          throw new RuntimeException("Image does not fit within max page width "+settings.maxWidth+paddingMessage+": "+rect.name+" "+width+"x"+height);
        }
        if(height>maxHeight&&(!settings.rotation||width>maxHeight)) {
          String paddingMessage=edgePadY?(" and Y edge padding "+paddingY+"*2"):"";
          throw new RuntimeException("Image does not fit within max page height "+settings.maxHeight+paddingMessage+": "+rect.name+" "+width+"x"+height);
        }
      }
    }
    ...
    if(settings.square) {
      ...
      if(!settings.silent) System.out.println();
      // Rects don't fit on one page. Fill a whole page and return.
      if(bestResult==null) bestResult=packAtSize(false,settings.maxWidth+adjustX,settings.maxHeight+adjustY,inputRects);
      sort.sort(bestResult.outputRects,rectComparator);
  ...
  static public enum FreeRectChoiceHeuristic{
    /**
     * BSSF: Positions the rectangle against the short side of a free rectangle into which it fits
     * the best.
     */
    BestShortSideFit,
    /**
     * BLSF: Positions the rectangle against the long side of a free rectangle into which it fits
     * the best.
     */
    BestLongSideFit,
