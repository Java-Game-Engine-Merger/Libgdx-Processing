...
public class ImageProcessor{
  ...
  /**
   * Returns a rect for the image describing the texture region to be packed, or null if the image
  ...
  protected Rect stripWhitespace(String name,BufferedImage source) {
    ...
    if(alphaRaster==null||(!settings.stripWhitespaceX&&!settings.stripWhitespaceY)) return new Rect(source,0,0,source.getWidth(),source.getHeight(),false);
    ...
    if(newWidth<=0||newHeight<=0) {
      ...
      else return new Rect(emptyImage,0,0,1,1,false);
    }
   ...
   * region. Splits are an int[4] that
  ...
  /**
   * Returns the pads, or null if the image had no pads or the pads match the splits. Pads are an
   * int[4] that has left, right,
   * top, bottom.
   */
  ...
  /**
   * Hunts for the start or end of a sequence of split pixels. Begins searching at (startX,
   * startY) then follows along the x or
   * y axis (depending on value of xAxis) for the first non-transparent pixel if startPoint is
   * true, or the first transparent
   * pixel if startPoint is false. Returns 0 if none found, as 0 is considered an invalid split
   * point being in the outer border
