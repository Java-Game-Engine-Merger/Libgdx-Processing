...
/**
 * A Batchable representing a rectangle and supporting zero or more Textures/TextureRegions, and
 * supporting color, position,
 * scale, and an origin offset.
 * <p>
 * By default, one texture is used. It may be subclassed to create a Batchable class that
 * supports zero or multiple textures and
 * additional attributes--see {@link #getNumberOfTextures()} and
 * {@link #addVertexAttributes(com.badlogic.gdx.utils.Array)
 * addVertexAttributes()}.
 * <p>
 * A Quad has fixed size, so its indices do not need to be recalculated for every draw call.
 *
 * @param <T> The type returned by the chain methods. The object must be able to be cast to this
 *            type.
 * @author cypherdare
 */
public abstract class Quad<T extends Quad<T>>extends FixedSizeBatchable{
  ...
  /**
   * Width and height must be set with {@link #size(float, float)}. If they are not set, they
   * default to the size of the first
   * texture region.
   */
  protected float width,height;
  /**
   * Whether the width and height have been set since the last call to {@link #refresh()}. If they
   * have not been set, they will
   * be set to match the size of the first texture region when drawing occurs.
   */
  protected boolean sizeSet;
  /**
   * The number of times the texture coordinates should be rotated clockwise by 90 degrees. Must
   * be positive.
   */
  ...
  protected Quad() {
    ...
    for(int i=0;i<regions.length;i++) regions[i]=new Region2D();
  }
  ...
  public boolean hasEquivalentTextures(Batchable other) {
    if(other instanceof Quad) {
      ...
      if(getNumberOfTextures()==1) return quad.textures[0]==textures[0];
  ...
  /**
   * Determines whether the position data has a Z component. Must return the same constant value
   * for every instance of the
   * class.
   * <p>
   * Overriding this method will produce a subclass that is incompatible with a FlexBatch that was
  ...
  /**
   * Determines whether the texture coordinate data has a third component (for TextureArray
   * layers). Must return the same
  ...
  /**
   * Resets the state of the object and drops Texture references to prepare it for returning to a
   * {@link Pool}.
   */
  @Override
  public void reset() {
  ...
  /**
   * Sets the texture and sets the region to match the size of the texture. If this Batchable
   * supports multi-texturing, multiple
   * subsequent calls to this method or {@link #textureRegion(TextureRegion)} will sequentially
   * set the textures in order.
   * <p>
  ...
  /**
   * Sets the UV region of the most recently applied texture. This must be called after a texture
   * or texture region has been set
   * with {@link #texture(GLTexture)} or {@link #textureRegion(TextureRegion)}. Note that
   * TextureRegions have a Y-down coordinate
   * system for UVs.
   * <p>
   * This method must not be called in a Batchable that supports zero textures.
   * 
  ...
  /**
   * Sets the UV region of the most recently applied texture, using texel units. This must be
   * called after a Texture or
   * TextureRegion has been set with {@link #texture(GLTexture)} or
   * {@link #textureRegion(TextureRegion)}. Note that
  ...
  /**
   * Sets the texture region. If this Batchable supports multi-texturing, multiple subsequent
  ...
  /**
   * Flips the UV region of the most recently applied texture. This must be called after a texture
   * or region has been set with
  ...
  /**
   * Flips the texture region(s) from their current state. Must be called after regions or texture
  ...
  /**
   * Sets the center point for transformations (rotation and scale), and for positioning. For
   * {@link Quad2D}, this is
   * relative to the bottom left corner of the texture region. For {@link Quad3D}, this is
