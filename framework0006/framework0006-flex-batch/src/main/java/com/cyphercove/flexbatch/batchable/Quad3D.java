 ...
 * two-dimensional position and color. It is designed to be drawn in 3D space, and is commonly
 * called a decal.
 * <p>
 * The default origin of a Quad3D is its center. Its origin is used for positioning, and as the
 * center of rotation and scaling.
 * <p>
 * Quad3D manages its own blending and ignores the FlexBatch's blend settings.
 * <p>
 * It may be subclassed to create a Batchable class that supports multiple textures and
 * additional attributes--see
 * {@link #getNumberOfTextures()} and {@link #addVertexAttributes(com.badlogic.gdx.utils.Array)
 * addVertexAttributes()}. Such a
 * subclass would not be compatible with a FlexBatch that was instantiated for the base Sprite
 * type.
 *
 * @author cypherdare
 */
public class Quad3D extends Quad<Quad3D> implements SortableBatchable{
  ...
  /**
   * Enables blending and sets the blend function parameters to a commonly used pair. Blending is
   * disabled by default.
   * 
   * @return This object for chaining.
   */
  ...
  /**
   * Sets the position of the center (plus current origin offset) of the texture region in world
   * space.
   * 
   * @return This object for chaining.
   */
  ...
  /**
   * Sets the rotation based on a direction vector and up vector. The input vectors <b>do not</b>
  ...
  /**
   * Sets the rotation based on a direction vector and up vector. The input vectors <b>must be</b>
   * normalized.
   * 
   * @return This object for chaining.
   */
  ...
  /**
   * Sets the rotation to look at the camera, relative to the current position. The quad's top
   * side will be oriented to match
   * the camera's.
   * 
   * @return This object for chaining.
   */
