...
/**
 * Draws batched {@link Batchable} objects, optimizing the drawing process by combining them
 * into a single Mesh. FlexBatch can be
 * customized to batch almost any kind of small object by defining how to draw the object in a
 * Batchable implementation. Most of
 * the available Batchable types in this library are designed for subclassing, to minimize the
 * work needed to define a Batchable.
 * A FlexBatch is instantiated using a Batchable class that defines the types of Batchables it
 * can draw. A FlexBatch must be
 * {@link #dispose() disposed of} when no longer used to avoid leaking memory.
 * <p>
 * Batchables can only be drawn between calls to {@link #begin()} and {@link #end()}. The
 * current queue of Batchables can be
 * flushed to screen immediately with a call to {@link #flush()}. Flushes also occur
 * automatically when {@link #end()} is called
 * or if any relevant OpenGL state changes or drawing changes are made (such as drawing a
 * Batchable with different Texture(s) than
 * the last, changing blend function parameters, changing the shader used, or changing the
 * projection matrix).
 * <p>
 * Batchable objects can be passed to the {@link #draw(Batchable)} method to queue them for
 * drawing. Alternatively, the
 * {@link #draw()} method can be called to acquire a Batchable matching the class passed to the
 * constructor parameter. This
 * Batchable can be immediately customized for drawing, and will be automatically queued for
 * drawing on the next call to any
 * FlexBatch method.
 * <p>
 * A single FlexBatch might be capable of drawing more than one type of Batchable. See
 * {@link #draw(Batchable)} for details.
 * <p>
 * Unlike SpriteBatch, no default shader is provided automatically. A call to
 * {@link #setShader(ShaderProgram)} must be made
 * before any drawing can occur. The attribute names to be used in the vertex shader are defined
 * by the Batchable. By default
 * FlexBatch automatically passes its combined transform and projection matrices to the shader
 * with the uniform name
 * {@code "u_projTrans"}. This can be customized by overriding {@link #applyMatrices()}. By
 * default, FlexBatch automatically
 * passes texture uniforms to the shader, with a count defined by the template Batchable class.
 * The texture uniforms are given the
 * names {@code "u_texture0"}, {@code "u_texture1"}, etc., but this can be customized by
 * overriding
 * {@link #applyTextureUniforms()}.
 * <p>
 * Other uniforms can be manually applied to the shader in between calls to {@link #begin()} and
 * {@link #end()}. Take care of when
 * these changes are made in relation to batch flushes, as only the most recently set uniforms
 * are used when a flush occurs.
 * <p>
 * <i>This API is based on SpriteBatch and NewSpriteBatch from the LibGDX project.</i>
 *
 * @param <T> The type of Batchable that is returned when acquiring one with {@link #draw()}.
 *            This must match the class type that
...
public class FlexBatch<T extends Batchable> implements Disposable{
  ...
  /**
   * Construct a FlexBatch capable of drawing the given Batchable type and other compatible
   * Batchables (ones with the same + *
   * VertexAttributes or subset of beginning VertexAttributes). The FlexBatch will not be limited
   * to FixedSizeBatchables.
   *
   * @param batchableType The type of Batchable that defines the VertexAttributes supported by
   *                      this FlexBatch, and the + *
   *                      default Batchable type drawn by the {@link #draw()} method.
   * @param maxVertices   The number of vertices this FlexBatch can batch at once. Maximum of
   *                      32767. If the Batchable is a
   *                      FixedSizeBatchable and 0 is used for {@code maxPrimitives}, this value
   *                      will be rounded down
   *                      to a multiple of the Batchable's size.
   * @param maxPrimitives The number of OpenGL primitives (lines or triangles) this FlexBatch can
   *                      batch at once, or 0
   *                      to optimize this FlexBatch to draw only FixedSizeBatchables. If the
   ...
   *             may create issues with minification (Proguard or R8) if the batchable class is
   *             obfuscated.
   */
  @Deprecated
   ...
   *                      this FlexBatch, and the + *
   *                      default Batchable type drawn by the {@link #draw()} method.
   * @param provider      Functional interface that generates a new instance of the Batchable
   *                      type. Used to generate an
  ...
  public FlexBatch(Class<T> batchableType,Batchable.Provider<T> provider,int maxVertices,int maxPrimitives) {
    ...
    if(maxVertices>32767) throw new IllegalArgumentException("Can't have more than 32767 vertices per batch: "+maxPrimitives);
    if(Modifier.isAbstract(batchableType.getModifiers())) throw new IllegalArgumentException("Can't use an abstract batchableType");
  ...
  /**
   * Returns the render context, which can be used to modify drawing parameters for the next batch
   * flush. Changes to
   * the render parameters will override any that have been set by batchables that are queued and
   * awaiting the next
   * flush, with the exception of the last batchable drawn with {@link #draw()}, if there is one.
   * Subsequent batchables
   * might reverse these changes in their
   * {@link Batchable#prepareContext(RenderContextAccumulator, int, int)} methods,
   * and therefore trigger a flush before they are queued. For example, {@link Quad3D}
   * manages blending parameters per batchable, and therefore setting blend parameters on the
   * RenderContextAccumulator
   * will have no effect.
   *
  ...
  /**
   * @return A Batchable that will automatically be queued for drawing upon the next call to
   *         draw(), flush(), or end(). The
  ...
  /**
   * Queues a Batchable for drawing. Certain characteristics of the supplied Batchable class must
   * match those of the
   ...
   * <li>{@link Batchable#prepareSharedContext(RenderContextAccumulator)} must perform equivalent
   * changes.
   * <li>This FlexBatch may have been set up to draw only FixedSizeBatchables in the constructor.
   * <li>The primitive type of the Batchable must be equivalent to that of the FlexBatch's
   * type.</li>
   * </ul>
   * The above criteria are not checked. If the supplied Batchable class's
   * {@link VertexAttributes}) do not match those of the
   * {@link #batchableType}, a different shader may be needed for this Batchable, and some vertex
   * data will go unused. The docs
  ...
  /**
   * Draws explicit vertex data, using only the render context and Texture parameter(s) of the
   * passed in FixedSizeBatchable. The
   * restrictions on the supplied Batchable class are the same as those in
   * {@link #draw(Batchable)}.
   *
   * @param batchable        A Batchable that defines the render context and textures to draw, but
   *                         not the actual vertex data.
   ...
   * @param count            The number of array elements to pass. This must be a multiple of the
   *                         size of the FixedSizeBatchable, and is not
   *                         checked.
   * @param vertexSize       The size of the vertices in the data (from the
   *                         {@link VertexAttributes}. Must be the same or smaller than
   *                         the size of the vertices for the Batchable type. If smaller, the data
   *                         for the excess vertex attributes will be
   *                         garbage, but this may be acceptable if the current shader doesn't use
   *                         them. It is assumed that the
   *                         VertexAttributes being drawn match the first of the VertexAttributes
   *                         of the Batchable type.
   */
  protected void draw(FixedSizeBatchable batchable,float[] explicitVertices,int offset,int count,int vertexSize) {
    ...
    if(this.vertexSize==vertexSize) {
      ...
      vertIdx+=copyCount;
      if(fixedIndices) indIdx+=(copyCount/this.vertexDataPerBatchable)*this.indicesPerBatchable;
      else {
      ...
      if(fixedIndices) indIdx+=(vertexCount*this.indicesPerBatchable)/this.verticesPerBatchable;
      else {
        int verticesPerBatchable=batchable.getVerticesPerBatchable();
  ...
  /**
   * Draws explicit vertex data, using only the render context and Texture parameter(s) of the
   * passed-in Batchable. This must
   * only be called if this FlexBatch is not limited to drawing FixedSizedBatchables. The
   * restrictions on the supplied Batchable
   * class are the same as those in {@link #draw(Batchable)}.
   * <p>
   * The batch must have enough total capacity for the entire set of vertices and triangles. This
   * is not checked.
   *
   * @param batchable        A Batchable that defines the render context and textures to draw, but
   *                         not the actual vertex data.
   ...
   * @param vertexDataCount  The number of array elements to pass. This must be the correct size
   *                         for the Batchable type, and is
   *                         not checked.
   * @param vertexSize       The size of the vertices in the data (from the
   *                         {@link VertexAttributes}. Must be the same or smaller than
   *                         the size of the vertices for the Batchable type. If smaller, the data
   ...
   * @param verticesCount    The number of array elements to pass. Must be a multiple of the size
   *                         of the primitive,
  ...
  protected void draw(Batchable batchable,float[] explicitVertices,int verticesOffset,int vertexDataCount,int vertexSize,
    ...
    if(verticesLength-vertIdx<vertexCount*this.vertexSize||(usesIndices&&indicesLength-indIdx<verticesCount)) flush();
    ...
    if(usesIndices) {
      ...
      for(int i=indIdx;i<upTo;i++) indices[i]+=startingVertex;
  ...
  /**
   * Flushes the same triangles drawn the last time a flush occurred. This is an expert method
   * with no error checks. It
   * provides a way to avoid re-submitting the batchable data for drawing if none of them have
   * changed since the last
   * flush and nothing in the render context has changed either (textures, blend modes, etc.).
   * Note that there are
   * several reasons the previous flush may have occurred other than a manual call to
   * {@link #flush()}, so care must
  ...
  /**
   * Skips ahead in draw submissions by the specified count of FixedSizeBatchables, which
   * effectively resubmits the
   * the same batchables from the previous flush with no modifications to render context
   * (textures, blend modes, etc.),
   * while avoiding all the overheads of calculating and copying vertex data. This is an expert
   * method with no error
   * checks. Note that there are several reasons the previous flush may have occurred other than a
   * manual call to
   * {@link #flush()}, so care must be taken to be aware of these and avoid them if necessary.
   * 
   * @param count The number of FixedSizeBatchables to assume are unchanged from the previous
   *              flush.
   */
  ...
  public ShaderProgram requireShader() {
    if(shader==null) throw new IllegalStateException("The ShaderProgram has not been assigned.");
    return shader;
  }

  /**
   * Sets the ShaderProgram to be used with this batch. Can be set to null to clear it, but this
   * must not be done
   * between {@link #begin()} and {$link #end()}.
   * 
   * @param shader The ShaderProgram to use, or null to clear it when not between {@link #begin()}
   *               and {$link #end()}.
   */
  public void setShader(ShaderProgram shader) {
    if(drawing) {
      if(shader==null) throw new IllegalStateException("Cannot set shader to null between begin() and end().");
      flush();
    }
    this.shader=shader;
    if(drawing) {
  ...
  /**
   * Called only while drawing. Recalculates the matrices and sets their values to shader
   * uniforms. The default implementation
   * combines the projection and transform matrices and sets them to a single uniform named
   * "u_projTrans".
   */
  ...
  /**
   * Sets shader uniform values for the textures. The default implementation uses the uniform name
   * "u_texture" with the texture
   * unit appended. For example, if the Batchable type supports two textures, uniforms will be set
   * for "u_texture0" and
   * "u_texture1".
   */
  protected void applyTextureUniforms() {
    ...
    for(int i=0;i<textureUnitUniforms.length;i++) getShader().setUniformi(textureUnitUniforms[i],i);
  ...
  /**
   * @return The number of batchables that can be drawn between flushes, or 0 if the batchable is
   *         not a {@link FixedSizeBatchable}.
   */
  public int getBatchableCapacity() {
    if(fixedIndices) return vertices.length/(vertexSize*verticesPerBatchable);
    return 0;
  }
