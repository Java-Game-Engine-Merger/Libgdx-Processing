...
/**
 * A Batchable representing a point drawn as a square after projection. Supports zero or more
 * Textures, but not
 * TextureRegions. Supports color, position and size (with shader attribute name
 * {@code "a_size"}. The square is centered
 * on the position.
 * <p>
 ...
 * {@link #addVertexAttributes(Array)
 * addVertexAttributes()}.
 * <p>
...
public abstract class Point<T> extends Batchable{
  ...
  public boolean hasEquivalentTextures(Batchable other) {
    if(other instanceof Point) {
      ...
      if(getNumberOfTextures()==1) return point.textures[0]==textures[0];
  ...
  /**
   * Note: Point uses generic attribute {@link AttributeOffsets#generic0} internally. Additional
   * generic attributes
