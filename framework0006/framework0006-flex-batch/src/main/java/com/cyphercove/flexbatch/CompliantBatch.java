...
/**
 * A {@link FlexBatch} that implements the {@link Batch} interface, so it is compatible with
 * Stage/Actor, BitmapFont,
 * ParticleEffect, Sprite, and NinePatch. It creates its own default ShaderProgram, which is
 * owned and is disposed automatically
 * with the CompliantBatch.
 * <p>
 * A CompliantBatch must be {@link #dispose() disposed of} when no longer used to avoid leaking
 * memory.
 * <p>
 * In addition to drawing Quad2Ds (or given subclass), if polygon support is specified in the
 * constructor, it may also draw
 * Poly2Ds (or matching subclass) by submitting them to {@link #draw(Batchable)}.
 * <p>
 * A subclass of Quad2D may be passed to the constructor to customize what is drawn
 * (multi-texturing or other attributes).
 * 
 * @param <T> The type of Quad2D that is returned when acquiring one with {@link #draw()}. This
 *            must match the class type that is
...
public class CompliantBatch<T extends Quad2D>extends FlexBatch<T> implements Batch{
  ...
  /**
   * Constructs a CompliantQuadBatch with a default shader and a capacity of 1000 quads that can
   * be drawn per flush. The default
   * shader is owned by the CompliantQuadBatch, so it is disposed when the CompliantQuadBatch is
   * disposed. If an alternate shader
   * has been applied with {@link #setShader(ShaderProgram)}, the default can be used again by
   * setting the shader to null.
   * 
   * @param supportPolygons Whether Poly2Ds are supported for drawing. The FlexBatch will not be
   *                        optimized for
   *                        FixedSizeBatchables.
   */
  ...
  /**
   * Constructs a CompliantQuadBatch with a default shader. The default shader is owned by the
   * CompliantQuadBatch, so it is
   * disposed when the CompliantQuadBatch is disposed. If an alternate shader has been applied
   * with
   * {@link #setShader(ShaderProgram)}, the default can be used again by setting the shader to
   * null.
   * 
   ...
   *                              is owned by the
   *                              CompliantQuadBatch, so it is disposed when the
   *                              CompliantQuadBatch is disposed. If an alternate shader has been
   *                              applied with {@link #setShader(ShaderProgram)}, the default can
   *                              be used again by setting the shader to null.
   * @param supportPolygons       Whether Poly2Ds are supported for drawing. The FlexBatch will
  ...
  public CompliantBatch(Class<T> batchableType,int maxVertices,boolean generateDefaultShader,boolean supportPolygons) {
    ...
    if(generateDefaultShader) {
      ...
      if(!defaultShader.isCompiled()) throw new IllegalArgumentException("Error compiling shader: "+defaultShader.getLog());
  ...
  /**
   * Returns the current ShaderProgram, which may be the default. For CompliantBatch, this is no
   * different than
  ...
  /**
   * Sets the ShaderProgram to be used with this batch. Can be set to null to restore use of the
   * default shader.
   * 
   ...
   * Does not affect the color of
   * anything drawn with {@link #draw()}, {@link #draw(Batchable)}, or
  ...
  /**
   * Sets the color used to tint images when they are added to the Batch. Default is equivalent to
   * Color.WHITE. Does not affect
   * the color of anything drawn with {@link #draw()}, {@link #draw(Batchable)}, or
   * {@link #draw(Texture, float[], int, int)}.
   */
  @Override
  ...
  /**
   * Sets the color used to tint images when they are added to the Batch. Default is
   * {@link Color#toFloatBits()
   * Color.WHITE.toFloatBits()}. Does not affect the color of anything drawn with {@link #draw()},
   * {@link #draw(Batchable)}, or
   * {@link #draw(Texture, float[], int, int)}.
   */
  @Override
