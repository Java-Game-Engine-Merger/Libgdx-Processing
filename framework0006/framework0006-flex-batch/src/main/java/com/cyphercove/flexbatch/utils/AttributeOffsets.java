...
public class AttributeOffsets{
  ...
  /**
   * Refreshes the mapping of aliases to offsets. Call if any {@link VertexAttribute#alias alias}
   * has been changed.
   */
  public void update() {
    byAlias.clear();
    for(VertexAttribute attribute:attributes) byAlias.put(attribute.alias,attribute.offset/4);
  }

  /**
   * Get the VertexAttribute offset, in float-size units, looking it up by its
   * {@link VertexAttribute#alias alias}.
   *
  ...
  /**
   * Get the VertexAttribute offset, in float-size units, looking it up by its index in the
   * VertexAttributes.
   *
   * @param attributeIndex The index of the VertexAttribute to retrieve.
   * @return The offset.
   * @throws IndexOutOfBoundsException If the given attribute index does not exist in the
   *                                   VertexAttributes.
   */
