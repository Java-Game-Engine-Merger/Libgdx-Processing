...
/**
 * Stores up pending GL state changes and textures to bind, and executes them on demand.
 * Minimizes actual state changes.
 * Remembers and can restore state between uses.
 * <p>
 * Pending state changes can be made at any time, but can only be applied by calling
 * {@link #executeChanges()} between calls to
...
public class RenderContextAccumulator{
  ...
  static final class State{
     ...
     * {@link RenderContextAccumulator#executeChanges()}. This reduces unnecessary state restoration
     * calls in
  ...
  /**
   * Begin tracking GL state. If any pending changes to state have been made, they will be applied
   * on the first call to
   * {@link #executeChanges()}.
   * <p>
   * This call must be matched with a call to {@link #end()}, which returns OpenGL states to their
   * defaults. The
   * {@link #executeChanges()} method may only be called in between {@link #begin()} and
   * {@link #end()}.
   */
  public void begin() {
  ...
  public boolean hasPendingChanges() {
    ...
    if(pending.blending) {
      if(pending.blendSrcFuncColor!=current.blendSrcFuncColor||pending.blendDstFuncColor!=current.blendDstFuncColor
        ||pending.blendSrcFuncAlpha!=current.blendSrcFuncAlpha||pending.blendDstFuncAlpha!=current.blendDstFuncAlpha) return true;
      if(pending.blendEquationColor!=current.blendEquationColor||pending.blendEquationAlpha!=current.blendEquationAlpha) return true;
    }
  ...
  /**
   * Applies the pending state changes and texture bindings to GL. This must be called in between
   * {@link #begin()} and
   * {@link #end()}.
   */
  public void executeChanges() {
    ...
    if(pending.depthTesting!=current.depthTesting) {
      if(pending.depthTesting) Gdx.gl.glEnable(GL20.GL_DEPTH_TEST);
    ...
    if(pending.blending!=current.blending) {
      if(pending.blending) Gdx.gl.glEnable(GL20.GL_BLEND);
    ...
    if(pending.blending) {
      ...
      if(pending.blendEquationColor!=current.blendEquationColor
        ...
        if(pending.blendEquationColor==pending.blendEquationAlpha) Gdx.gl.glBlendEquation(pending.blendEquationColor);
        else Gdx.gl.glBlendEquationSeparate(pending.blendEquationColor,pending.blendEquationAlpha);
      }
    }
  ...
  /**
   * Returns actual OpenGL states to defaults. The blend function parameters, depth test function
   * parameters, and culled face
  ...
  /**
   * Sets the depth test function and a depth range of 0 to 1. These parameters will only be
   * applied if depth testing is
   * enabled.
   * 
  ...
  /**
   * Sets the depth test function and range. These parameters will only be applied if depth
   * testing is enabled.
   * 
   * @return Whether the pending depth function state or parameters were changed.
   */
   ...
   * 
   * @return Whether the pending blend parameters were changed while the pending blending state is
   *         true.
   */
  ...
  /**
   * Sets the blendFunc, using separate parameters for the color and alpha components. They will
   * only be applied when blending
   * is enabled.
   * 
   * @return Whether the pending blend parameters were changed while the pending blending state is
   *         true.
   ...
   * 
   * @return Whether the pending blend equations were changed while the pending blending state is
   *         true.
   */
  ...
  /**
   * Sets the blend equations, using separate equations for the color and alpha components. They
  ...
  /**
   * Sets which face(s) is culled when face culling is enabled. It will only be applied when face
   * culling is enabled.
   * 
  ...
  public boolean setTextureUnit(GLTexture texture,int unit) {
    if(pending.textureUnits.get(unit)!=texture) {
      if(texture==null) pending.textureUnits.remove(unit);
      else pending.textureUnits.put(unit,texture);
      return true;
    }
    return false;
  }
  ...
  /**
   * Cancels all pending texture bindings and drops all Texture references held by
