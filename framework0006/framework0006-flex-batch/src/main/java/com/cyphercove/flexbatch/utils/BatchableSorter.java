...
/**
 * Sorts 3D {@link Batchable Batchables} to ensure proper render order before passing them to a
 * {@link FlexBatch}.
 * <p>
 * Opaque Batchables are sorted by texture configuration to minimize flushes and drawn first.
 * Blended Batchables are sorted by
...
public class BatchableSorter<T extends Batchable&SortableBatchable>{
   ...
   *
   * @param camera                          Initial camera to use for sorting distance.
   * @param opaqueInitialTextureCapacity    The initial number of backing arrays created for
   *                                        opaque Batchables. One is
   *                                        used for each unique texture that is simultaneously
   *                                        queued. This should be
   *                                        as high as the expected number of unique textures for
   *                                        opaque Batchables to
   *                                        prevent potential stutters from adding new backing
   *                                        arrays during queuing.
   * @param opaqueInitialCapacityPerTexture The initial capacity of each backing array of opaque
   *                                        Batchables. One is
   *                                        created for each unique texture that is simultaneously
   *                                        queued. A larger
   *                                        initial capacity may prevent backing array resizes
   *                                        during queuing, and so
   *                                        may reduce the potential for stutters.
   * @param blendedInitialCapacity          The initial capacity of the backing array that stores
   *                                        blended Batchables before they
   *                                        are flushed or drawn. A larger initial capacity may
   *                                        prevent backing array resizes
   *                                        during queuing, and so may reduce the potential for
   *                                        stutters.
   */
  public BatchableSorter(Camera camera,
  ...
  public void clear() {
    for(ObjectSet<T> set:opaqueBatchables.values()) objectSetPool.free(set);
    opaqueBatchables.clear();
    blendedBatchables.clear();
  }

  /**
   * Sort (if necessary) and draw the queued Batchables without clearing them. Must be called in
   * between
  ...
  public void draw(FlexBatch<T> flexBatch) {
    ...
    for(ObjectSet<T> set:opaqueBatchables.values()) for(T batchable:set) flexBatch.draw(batchable);
    for(T batchable:blendedBatchables) flexBatch.draw(batchable);
  }

  /**
   * Sort (if necessary), draw, and clear references to the queued Batchables. Must be called in
   * between
   * {@link FlexBatch#begin()} and {@link FlexBatch#end()}.
   *
   * @param flexBatch The batch to draw the batchables with.
