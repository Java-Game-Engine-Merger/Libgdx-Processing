...
/**
 * A utility to output BitmapFontData to a FNT file. This can be useful for caching the result
 * from TrueTypeFont, for faster load times.
 * <p>
 * The font file format is from the AngelCodeFont BMFont tool.
 * <p>
 * Output is nearly identical to the FreeType settting in the Hiero tool
 * {@Link com.badlogic.gdx.tools.hiero.Hiero}. BitmapFontWriter gives more flexibility, eg
 * borders and shadows can be used. Hiero is able to avoid outputting the same glyph image more
 * than once if multiple character codes have the exact same glyph.
 * 
...
public class BitmapFontWriter{
  ...
  /**
   * Sets the AngelCodeFont output format for subsequent writes; can be text (for libGDX) or XML
   * (for other engines, like Pixi.js).
   *
  ...
  /**
   * The font "info" line; everything except padding and override metrics are ignored by libGDX's
   * BitmapFont reader, it is otherwise just useful for clean and organized output.
   */
  ...
  private static String quote(Object params,boolean spaceAfter) {
    if(getOutputFormat()==OutputFormat.XML) return "\""+params.toString().trim()+"\""+(spaceAfter?" ":"");
    else return params.toString();
  }

  /**
   * Writes the given BitmapFontData to a file, using the specified <tt>pageRefs</tt> strings as
   * the image paths for each texture page. The glyphs in BitmapFontData have a "page" id, which
   * references the index of the pageRef you specify here.
   * <p>
   * The FontInfo parameter is useful for cleaner output; such as including a size and font face
   * name hint. However, it can be null to use default values. libGDX ignores most of the "info"
   * line when reading back fonts, only padding is used. Padding also affects the size, location,
   * and offset of the glyphs that are output.
   * <p>
   * Likewise, the scaleW and scaleH are only for cleaner output. They are currently ignored by
   * libGDX's reader. For maximum compatibility with other BMFont tools, you should use the width
   * and height of your texture pages (each page should be the same size).
   *
   * @param fontData               the bitmap font
   * @param pageRefs               the references to each texture page image file, generally in
   *                               the same folder as outFntFile
  ...
  public static void writeFont(BitmapFontData fontData,String[] pageRefs,FileHandle outFntFile,FontInfo info,
    ...
    OutputFormat fmt=getOutputFormat();
   ...
   *
   * The specified pixmaps are written to the parent directory of <tt>outFntFile</tt>, using that
   * file's name without an extension for the PNG file name(s).
   *
  ...
  public static void writeFont(BitmapFontData fontData,Pixmap[] pages,FileHandle outFntFile,FontInfo info) {
    String[] pageRefs=writePixmaps(pages,outFntFile.parent(),outFntFile.nameWithoutExtension(),null);

    // write the font data
    writeFont(fontData,pageRefs,outFntFile,info,pages[0].getWidth(),pages[0].getHeight(),null);
  }

  /**
   * A utility method to write the given array of pixmaps to the given output directory, with the
   * specified file name. If the pages array is of length 1, then the resulting file ref will look
   * like: "fileName.png".
   *
   * If the pages array is greater than length 1, the resulting file refs will be appended with
   * "_N", such as "fileName_0.png", "fileName_1.png", "fileName_2.png" etc.
   *
   ...
   */
  public static String[] writePixmaps(Pixmap[] pages,FileHandle outputDir,String fileName,DistanceFieldGenerator distanceFieldGenerator) {
    ...
    for(int i=0;i<pages.length;i++) {
      ...
      if(distanceFieldGenerator!=null) {
        BufferedImage inImage=(BufferedImage)new ImageIcon(pages[i].getPixels().array()).getImage();
        BufferedImage outImage=distanceFieldGenerator.generateDistanceField(inImage);
        //      var pixmap=new Pixmap(outImage.getWidth(),outImage.getHeight(),Format.RGBA8888);
        //      pixmap.setPixels(outImage.getRGB(0,0,outImage.getWidth(),outImage.getHeight(),null,0,outImage.getWidth()))
        //     int[] array= outImage.getRGB(0,0,outImage.getWidth(),outImage.getHeight(),null,0,outImage.getWidth());
        //      pixmap.setPixels(bufferedImageToPixmap(outImage));
        pages[i]=bufferedImageToPixmap(outImage);
      }
  ...
  public static Pixmap bufferedImageToPixmap(BufferedImage image) {
    ...
    for(int y=0;y<height;y++) {
      for(int x=0;x<width;x++) {
        ...
        int rgba8888=((argb>>8)&0xffffff00)|((argb>>24)&0x000000ff);
  ...
  /**
   * A convenience method to write pixmaps by page; typically returned from a PixmapPacker when
   * used alongside FreeTypeFontGenerator.
   *
   ...
   */
  public static String[] writePixmaps(Array<Page> pages,FileHandle outputDir,String fileName,DistanceFieldGenerator distanceFieldGenerator) {
    Pixmap[] pix=new Pixmap[pages.size];
    for(int i=0;i<pages.size;i++) {
      pix[i]=pages.get(i).getPixmap();
    }
    return writePixmaps(pix,outputDir,fileName,distanceFieldGenerator);
  }
}
