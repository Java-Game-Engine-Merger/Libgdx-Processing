...
public class KryoNettyInitializer extends ChannelInitializer<SocketChannel>{
  ...
  public KryoNettyInitializer(Endpoint endpoint) {
    ...
    if(endpoint.getKryoNetty().getExecutionThreadSize()>0) this.executorGroup=new DefaultEventExecutorGroup(endpoint.getKryoNetty().getExecutionThreadSize());
    else this.executorGroup=null;
  }
  ...
  public void initChannel(SocketChannel ch) {
    ...
    if(endpoint.getKryoNetty().isUseLogging()) pipeline.addLast("logging-handler",new LoggingHandler(LogLevel.INFO));
