...
public final class UkcpServerChannel extends AbstractNioMessageChannel implements ServerChannel,Runnable{
  ...
  private final ReusableIterator<Map.Entry<SocketAddress,UkcpServerChildChannel>> childChannelMapItr=childChannelMap.entrySet().iterator();

  private final ReItrHashMap<SocketAddress,CloseWaitKcp> closeWaitKcpMap=new ReItrHashMap<>();

  private final ReusableIterator<Map.Entry<SocketAddress,CloseWaitKcp>> closeWaitKcpMapItr=closeWaitKcpMap.entrySet().iterator();
  ...
  void doCloseChildChannel(UkcpServerChildChannel childChannel) {
    ...
    if(isActive()&&ukcp.getState()!=-1&&ukcp.checkFlush()) {
      ...
      int current=Utils.milliSeconds();
      closeWaitKcpMap.put(ukcp.channel().remoteAddress(),new CloseWaitKcp(ukcp,current+Consts.CLOSE_WAIT_TIME));
